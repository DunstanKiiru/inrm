...
class ControlController extends Controller
{
    public function index(Request $r){
        ...
        if($s=$r->query('q')) $q->where('title','ilike',"%$s%");
    ...
    public function store(Request $r){
        $data=$r->validate([
            ...
            'nature'=>'nullable|string','type'=>'nullable|string','frequency'=>'nullable|string',
            'owner_id'=>'nullable|exists:users,id','status'=>'nullable|string'
        ]);
        return Control::create($data);
    }
    public function show(Control $control){ return $control->load(['category','owner','testPlans.executions']); }
