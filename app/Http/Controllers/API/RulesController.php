...
class RulesController extends Controller
{
    public function index(Request $r)
    {
        ...
        if ($r->filled('enabled')) $q->where('enabled', (bool)$r->input('enabled'));
    ...
    public function store(Request $r)
    {
        ...
        $id = DB::table('tpr_rules')->insertGetId([
            ...
            'description_template'=> $data['description_template'] ?? 'Rule {{rule_id}} triggered for {{vendor_code}}: {{count}} in {{window_days}} days ({{metric}}={{matched_code}}).',
    ...
    public function update($id, Request $r)
    {
        ...
        DB::table('tpr_rules')->where('id',$id)->update(array_merge($data, [
            'scope'=> array_key_exists('scope', $data) ? json_encode($data['scope']) : DB::raw('scope'),
            'updated_at'=> now()
        ]));
        return DB::table('tpr_rules')->where('id',$id)->first();
    }
    ...
    public function audit(Request $r)
    {
        $q = DB::table('tpr_rule_audit as a')
            ...
            ->select('a.*','r.type','r.metric as rule_metric','r.code_pattern','r.window_days','r.threshold');
        if ($r->filled('rule_id')) $q->where('a.rule_id',$r->input('rule_id'));
        if ($r->filled('vendor_id')) $q->where('a.vendor_id',$r->input('vendor_id'));
        if ($r->filled('days')) $q->where('a.triggered_at','>=', now()->subDays((int)$r->input('days'))->toDateTimeString());
