        ...
        foreach ($rules as $rule) {
            ...
            foreach ($vendors as $v) {
                ...
                if ($rule->type === 'KRI_ALERTS_IN_WINDOW' || $rule->metric === 'kri') {
                    $q = DB::table('tpr_vendor_kri_measures')
                ...
                if ($rule->type === 'SLA_BREACHES_IN_WINDOW' || $rule->metric === 'sla') {
                    $q = DB::table('tpr_vendor_sla_measures')
                        ->where('vendor_id', $v->id)
                        ->where('measured_at', '>=', $winStart)
                        ->where('status', 'breach');
                ...
                foreach ($matched as $m) {
                    ...
                    elseif ($rule->action === 'rim_event_only' && Schema::hasTable('rim_events')) {
                        DB::table('rim_events')->insert([
                            ...
                            'occurred_at' => now(),
                            'metrics' => json_encode(['rule_id'=>$rule->id,'vendor_id'=>$v->id,'metric'=>$m['metric'],'code'=>$m['code'],'count'=>$m['count']]),
                            'created_at' => now(),
                            'updated_at' => now()
                        ]);
