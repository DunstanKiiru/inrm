...
class KriEvaluateBreaches extends Command {
  ...
  public function handle(){
    ...
    foreach(Kri::with('readings')->get() as $kri){
      ...
      $warn = $kri->warn_threshold; $alert = $kri->alert_threshold;
      $level = null;
      if($kri->direction === 'higher_is_better'){
        if($val < $alert && $alert !== null) $level='alert';
        elseif($val < $warn && $warn !== null) $level='warn';
      ...
      if($level){
        KriBreach::create([
          'kri_id'=>$kri->id,'reading_id'=>$reading->id,'level'=>$level,
